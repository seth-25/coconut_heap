...
float my_ts_euclidean_distance_bound(float *t, float *s, int size, float bound) {
...
//);
//
//int coconut_plus __P((int, int,int,int, int, int, char *, char *, int));
//void show __P((const char *, DBT *, DBT *));


int bulk_dbt_init(DBT *bulk, int itemcount) {
 ...
 *
 */
DB_ENV *env_init(const char *home, const char *progname, u_int cachesize) {
    DB_ENV *dbenv;
    int ret;
...
/*
dbenv;          The already-opened database environemnt
dbpp;           'out': return the primary db handle.
dups;           #duplicate per key in primary database.
secondaryflag;  If true, create the secondary database.
pagesize;       Create the database with this page size.
*/
int db_init(DB_ENV *dbenv, DB **dbpp, int dups, int secondaryflag, int pagesize) {
...
//return;
    ...
    if (indexing != 1) {
        ...
        if ((ret = dbp->open(dbp, NULL, DATABASE, NULL, DB_BTREE, DB_RDONLY, 0664)) != 0) {
            dbp->err(dbp, ret, "open: %s", DATABASE);
            return (1);
        }
        ...
        for (cnt = 0; cnt < queriesno; ++cnt) {
            ...
            xi = (long long unsigned int *) data.data;
...
//            //ts_print(data.data, 256);
            ...
            mindist = my_ts_euclidean_distance((float *) qts, (float *) mqts, lenz);
            //printf("\nMINDIST %f",mindist);
...
//            if (mindist != 0) {
            ...
            while ((ret = dbcp->get(dbcp, &key, &data, DB_NEXT)) == 0 && prevposition < (range * 2)) {
                ...
                dist = my_ts_euclidean_distance((float *) qts, (float *) mqts, lenz);
                // 堆里只留k个
                if (heap_size(heap) < k) {
                    heap_push(heap, mqts, dist);
                } else if (heap_top(heap) > dist) {
                    heap_pop(heap);
                    heap_push(heap, mqts, dist);
                }
                ...
                if (heap_size(heap) >= k && dist < topdist) {
                    topdist = dist;
                }
                prevposition++;
            }
            double k_avdist = 0;
            for (int i = 0; i < k; i++) {
                k_avdist += heap_top(heap);
                heap_pop(heap);
            }
            k_avdist /= k;
            printf("近似 最小dist: %lf 平均dist: %lf\n", mindist, k_avdist);

            avdistapprox += k_avdist;
...
int main(int argc, int **argv) {
    ...
    int memory = atoi((char *) argv[2]);
    int exact = atoi((char *) argv[3]);
    int queriesnumber = atoi((char *) argv[4]);
    int indexing = atoi((char *) argv[5]);
    int leafsize = atoi((char *) argv[6]);
    char *dataset = (char *) argv[7];
    char *queries = (char *) argv[8];
    int lenz = atoi((char *) argv[9]);
    int k = atoi((char *) argv[10]);

    printf("\nCOCONUT+>>Indexing %s\n", dataset);
    return (coconut_plus_heap(size, memory, exact, queriesnumber, indexing, leafsize, dataset, queries, lenz, k) == 1
